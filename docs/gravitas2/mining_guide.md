...
*   **IN-GAME GUIDE IS KING:** **Always prioritize and cross-reference this information with your in-game TFC Guidebook (especially its "Ores and Minerals," "Gregtech Veins," and "Mineral Hints" sections) and JEI (Just Enough Items).** These will provide the most accurate details for your specific pack version.
...
| Limonite (TFC)     | `Limonite` (Iron ore)                                              | Near Surface  | `Shale`, `Claystone`, `Limestone`, `Conglomerate`, `Dolomite`, `Chert`, `Chalk` (Sedimentary) | `Limonite Nuggets`; `Andesite` pebble in `Claystone` rock | Large TFC iron veins.                                  |
