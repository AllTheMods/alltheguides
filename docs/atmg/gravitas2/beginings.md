...
*   **Drink Safely:** Drink `Fresh Water` (++rbutton++ on a source block or from a container) from anything *but* `Oceans`. Ocean water is salty and will only make your thirst worse.
...
### Resource Collection
*   **`Pebbles`:** Pick up `Pebbles` you see along the ground by ++rbutton++ or breaking them. You'll get different types of stones and even small `Ore` pieces depending on the geological region.
...
*   **`Stone Javelin`:**
    *   A primitive Piercing weapon that can be thrown (hold ++rbutton++ to charge, release to throw) or used in melee. Useful for early hunting and fishing.
...
*   **`Prospector's Pick (ProPick)`:** Once you have `Copper`, crafting a `Copper ProPick` is a high priority. ++rbutton++ the ground with it to get readings of nearby `Ore` types and their richness. This is crucial for finding larger, underground `Ore` veins.
...
*   **Non-Gravity Building Blocks:**
    ...
    *   **`Wattle and Daub`:** A versatile early building option.
        ...
        2.  ++rbutton++ the `Logs` with `Sticks` to create the wattle framework. You can arrange sticks in different patterns.
        3.  ++rbutton++ the framework with `Daub` to make it solid.
...
*   **Soup:** A common early recipe. Made from 3-5 `Fruits`, `Vegetables`, or `Meats` in a pot of `Water`. When done, the water turns red. ++rbutton++ with a `Bowl` to retrieve `Soup`. Combines multiple nutrients.
...
4.  **Cast the Tool Head:**
    *   Once the `Pit Kiln` is done and the `Ceramic Vessel` with molten `Copper` is retrieved, ++rbutton++ with it in hand to open the Casting Interface.
...
5.  **Cool and Extract:** Remove the filled mold and let it cool down. Once cool, ++sneak++ + ++rbutton++ the mold (or place it in a crafting grid) to extract your `Copper Pickaxe Head` and (usually) retrieve the mold.
...
3.  **Create Anvil:** Place the `Raw Stone` block you've obtained and simply ++rbutton++ its top face with any `Hammer` (e.g., `Stone Hammer`). The block will transform into a `Stone Anvil`.
*   This `Stone Anvil` is Tier 0, the lowest tier. It can only be used to `Weld` Tier I ingots (like `Copper`). Check "Primitive Anvils" in the TFC Guidebook for more info.
...
*   **`Large Ceramic Vessel`:** Knap `Clay` into a `Large Vessel` and fire it.
    ...
    *   **Food Preservation:** Sealing food inside (++shift++ + ++rbutton++ with an empty hand) significantly slows down decay.
...
3.  **`Loom`:** Craft a `Loom`.
    *   Place the `Loom` down.
    *   ++rbutton++ it with `Wool Yarn` (or `String`) to load it. (16 `Wool Yarn` for `Wool Cloth`, 24 `String` for `Silk Cloth`).
    *   Once it has enough, hold ++rbutton++ on the `Loom` to work it.
    *   When finished, ++rbutton++ to get your `Wool Cloth` or `Silk Cloth`.
...
4.  **Soak Hide (Limewater):** Place `Raw Hide` into the `Limewater Barrel`, seal it (++shift++ + ++rbutton++ with an empty hand), and wait for it to process into `Soaked Hide`.
5.  **Scrape Hide:**
...
Really. Stay away. They are not early-game friendly and will destroy you and your surroundings.
